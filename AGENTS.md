...
**Scheduling (2026-07-03):** a CLOUD Claude routine can't reach the Mac's logged-in Chrome/Keychain, so the upload MUST run locally. `yt_studio_upload.py drain-queue` uploads every `*.mp4` in `/Volumes/SSK SSD/fromthelogo-cache/upload-queue/` as an auto-metadata DRAFT (headed, in the GUI session — Keychain works), then moves it to `uploaded/`. Schedule it with the launchd LaunchAgent in `tools/launchd/` (headed runs fine under a LaunchAgent because it's in the Aqua session; requires Mac on + user logged in, screen may be locked). Producer side: the render step just `cp`s finished videos into the queue dir. Shorts metadata is intentionally lean now — title + 1-2 hashtags, ALWAYS including #caitlinclark and #wnba, tags capped at 5 (Shorts are discovered by title+hashtags, not tags); long-form keeps the full description + 12-18 tags.

**Publish-from-draft (2026-07-03):** `yt_studio_upload.py publish --title T [--visibility public|unlisted] [--when "YYYY-MM-DD HH:MM"] [--confirm]`. DRY-RUN by default (reaches Visibility, selects target, screenshots, STOPS); `--confirm` clicks Publish. Mechanics: find the draft row by title (both tabs), CLICK its `#video-title` (drafts have no /video/ href — the id shows up as `?udvid=<id>` and clicking opens the publish WIZARD, not the edit page). The wizard steps (Details/Ad suitability/Video elements/Checks/Visibility) — Next is BLOCKED on "Ad suitability" by the self-rating, so DON'T Next through; click the "Visibility" STEP TAB directly (`get_by_role("tab", name="Visibility")`), then the `tp-yt-paper-radio-button[name="PUBLIC"|"UNLISTED"|"PRIVATE"]` radio, then `#done-button` ("Publish"). Verified to the publish-ready state on a draft; the final `--confirm` click is user-gated (publishing = public, never automatic). Skill: `ftl-youtube-draft` (~/.claude/skills/) chains produce → draft → publish.

**Daily backlog + hourly scheduler (2026-07-03):** LIBRARY_DIR (`/Volumes/SSK SSD/fromthelogo-cache/video-library/`) holds approved, ready videos (FIFO by mtime). `yt_studio_upload.py post-next` takes the OLDEST one, uploads it as an auto-metadata DRAFT, moves it to `posted/`, and fires a macOS notification (`_notify` via osascript, works from the launchd GUI session). One video per run. Schedule = `tools/launchd/com.ftl.yt-post-hourly.plist` (StartCalendarInterval at :00 for hours 8-22 = 15 drafts/day). DRAFT-ONLY by design (user reviews + publishes); nothing auto-publishes.
- **Google "Verify it's you" challenge:** appears intermittently under heavy automation volume (it blocked a mid-session upload at the title field). `needs_verify(page)` detects it; upload() exits with `NEEDS_VERIFY` (screenshot `needs-verify`); `post-next` catches that, KEEPS the video in the library (doesn't consume it), notifies, and next hour retries. `verify-identity` mode opens a headed window and holds up to 8 min for the USER to clear the prompt (identity/2FA — automation must not). A 1/hour cadence keeps re-triggering rare. This is the main reliability caveat for unattended runs.
- **Ad-suitability self-rating** (`rate_ad_suitability`) runs best-effort after upload to self-certify clean content (publish/monetization-ready) — but is UNVERIFIED (the verify prompt blocked the test run); guarded so it never breaks the draft. Revisit when the form is reachable on a fresh upload.

**Researcher (2026-07-03):** `tools/ftl_research.py` scans a curated list (`research/sources.json`: youtube_podcasts + x_accounts + keywords + formats) for viral clips/ideas. `report` uses yt-dlp NON-flat (capped `--playlist-end`, ~2.3s/video — flat-playlist omits view_count) to rank recent uploads by views, splits into WNBA/Caitlin (clip first) vs most-viral NBA-crossover pod clips (no keyword filter — viral = clippable, the algorithm finds the audience), tags a suggested format, and writes `research/YYYY-MM-DD-shorts-research.md`. `x-plan` prints the X /media URLs to scan in the logged-in browser (X engagement needs auth; phase 2). Confirmed working: Robin Lundberg WNBA episodes (3.6-21k views) + PodcastP/GilsArena/ClubShayShay NBA clips (59k-492k). Feeds the split-short pipeline; new formats (slow-mo/close-up CC highlight = top performer; CC play ranking) are noted in sources.json but need separate production builds. Note: ClubShayShay surfaces non-basketball celeb interviews — refine NBA relevance filter if noisy.

**X viral-clip scanner (2026-07-03):** `ftl_research.py x-scan` drives the cloned Tales profile (headed, logged-in X session) over each x_account's MAIN PROFILE timeline (NOT /media — that's a grid with no inline views; the profile timeline has article-layout posts with view counts) + the user's bookmarks, collects VIDEO posts (`[data-testid="videoComponent"]`), parses views from the analytics-link aria-label ("N views"), filters bookmarks to a SPORTS keyword set, ranks by views, writes `research/YYYY-MM-DD-x-clips.md`. Confirmed: 127 posts, NBA clips 1-3M views (@TheHoopCentral, @TheDunkCentral). GOTCHA handles: **NBA Central = @TheDunkCentral** (kept the legacy Dunk Central @ after rebrand); @DunkCentral is a separate dunk-video account; both @DunkCentral and @LegionHoops return 0 (post mostly text/screenshots/quote-tweets, not native video — needs a quote-tweet/media-grid path to capture). Uses yt_studio_upload._persistent for the session (X rejects headless too).

**Persistent browser window (2026-07-03):** `yt_studio_upload.py keep-open` launches ONE long-lived headed clone-profile window with a CDP endpoint on :9222 and holds it. `ctx()` now tries `connect_over_cdp(:9222)` FIRST — every command (upload/list/verify/publish/post-next) ATTACHES to that warm window instead of launching its own Chrome. `_CdpCtx.close()` only closes the pages that command opened (never the shared window). Why: relaunching Chrome per command was slow AND the repeated launches were the main trigger for Google's intermittent "Verify it's you" challenge; a single stable session is far less bot-like. Run `keep-open` in the background (or launchd) and leave it up. If :9222 isn't listening, commands fall back to launching their own window as before. Only ONE clone-profile Chrome can hold the profile lock, so all commands MUST share via CDP while keep-open runs (they do). Verified: status attached over CDP (logged in: True), no new browser launched.

**ftl-split-short skill — two robustness fixes (2026-07-04):** (1) `--topgravity top|center` — for VERTICAL talking-head sources the default center-crop cut the subject's head off the top half; `top` anchors the crop at the frame top (pure y=0). (2) **Sentence-boundary snap** — the script now cuts a 6s buffer past `--topend`, transcribes FIRST, and if the requested end lands mid-sentence it extends to the next sentence-ending word (`. ! ? ” "`) within the buffer, else keeps it; trims top/voice/bottom + captions to the snapped end and prints `[sentence-snap] REQ -> ACTUAL (full thought)`. So `--topend` is now a rough estimate — cuts never end mid-thought (this bit us on cc-not-a-robot, ended at "…I do put on and"). Skill file: `~/.claude/skills/ftl-split-short/build_split_short.py` (SSD dup at `_external/sophie-podcast/` kept in sync). Verified: end 22.1s auto-snapped to 28.1s ending on a complete sentence.

**ftl-split-short — dangling-word caption fix (2026-07-04):** the sentence-snap pad (+0.25s past the sentence-ending word) could pull the NEXT word's start into the caption filter, leaving a trailing dangling word on screen (e.g. "...ALL THE TIME. AND"). Fixed the caption word filter from `s<D0-0.05` (keep words STARTING before end) to `e<=D0+0.05` (keep words ENDING within the cut) — a word that spans the snap point is dropped. Protects every short.

**Researcher context-mode (2026-07-04):** `ftl_research.py x-scan` now reads BOTH viral video clips AND text. Accounts flagged `"context": true` in sources.json (@kenswift, @MickTalksHoops) have EVERY post captured (text up to 280 chars + has_video flag), not just video — their text is narrative context on the Caitlin world and they surface podcast clips to pull. Report splits into "Viral video clips — clip these (ranked)" and "Caitlin Clark world — context & story leads" (⏯ marks a post with a pullable clip). Normal accounts stay video-only. Mick Talks Hoops YouTube (@MickTalksHoops) is in youtube_podcasts (wnba lane) for his Caitlin news coverage. User: read these for context, don't have to clip them every time.
