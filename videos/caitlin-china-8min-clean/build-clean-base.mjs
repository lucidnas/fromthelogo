...
const vo = path.join(production,"audio/chronological-recap-v6-josh-normal/vo-master-stage-pauses.mp3");
const music = path.join(production,"audio/music/Illusions - Anno Domini Beats.mp3");
const college = path.join(production,"sources/stage-progression/college-iowa-deep-three-i7aM979td7w.mp4");
const wnba = path.join(production,"sources/stage-progression/wnba-fever-deep-three-jANcTPAOIYQ.mp4");
const usa = path.join(production,"sources/stage-progression/usa-qualifying-deep-three-rOWGkfWSmgc.mp4");
for (const file of [game,press,vo,music,college,wnba,usa]) if (!fs.existsSync(file)) throw new Error(`Missing required bundled asset: ${file}`);
...
function renderStageFullscreen(operation,duration,file){
  const clip=path.join(production,operation.sourceAsset.path);
  if(!fs.existsSync(clip)) throw new Error(`Missing full-screen stage asset: ${clip}`);
  run(["-y","-hide_banner","-loglevel","error","-ss",String(operation.source.in),"-to",String(operation.source.out),"-i",clip,"-vf",`${common},tpad=stop_mode=clone:stop_duration=${duration.toFixed(3)}`,"-frames:v",String(framesFor(duration)),...encode,file]);
}
function renderGraphicPoster(operation,duration,file){
  const graphic=path.join(production,operation.sourceAsset.path);
  if(!fs.existsSync(graphic)) throw new Error(`Missing graphic poster: ${graphic}`);
  const fade=Math.min(operation.fadeIn??0.3,duration/3);
  run(["-y","-hide_banner","-loglevel","error","-loop","1","-i",graphic,"-vf",`${common},fade=t=in:st=0:d=${fade},trim=duration=${duration}`,"-frames:v",String(framesFor(duration)),...encode,file]);
}
for (const [beatIndex,beat] of edl.beats.entries()) {
  ...
  for (const [opIndex,operation] of beat.pictureOperations.entries()) {
    ...
    } else if(operation.kind==="stage-fullscreen-native"){
      renderStageFullscreen(operation,duration,file);
    } else if(operation.treatmentType==="graphic-poster"){
      renderGraphicPoster(operation,duration,file);
...
const pressStart=edl.beats.find((beat)=>beat.id==="B17-COACH-RECEIPT").program.in;
const stageOps=edl.beats.find((beat)=>beat.id==="B00-INTRO").pictureOperations.filter((op)=>op.kind==="stage-fullscreen-native");
const stageDelays=stageOps.map((op)=>Math.round(op.programRelative.in*1000));
run(["-y","-hide_banner","-loglevel","error","-i",picture,"-i",vo,"-i",press,"-i",college,"-i",wnba,"-i",usa,"-stream_loop","-1","-i",music,"-filter_complex",`[1:a]aresample=48000,loudnorm=I=-16:TP=-1.5:LRA=11[vo];[2:a]atrim=start=35.48:end=41.92,asetpts=PTS-STARTPTS[p1];[2:a]atrim=start=120.36:end=138.18,asetpts=PTS-STARTPTS[p2];[2:a]atrim=start=145.72:end=167.14,asetpts=PTS-STARTPTS[p3];[p1][p2][p3]concat=n=3:v=0:a=1,adelay=${Math.round(pressStart*1000)}|${Math.round(pressStart*1000)},loudnorm=I=-16:TP=-1.5:LRA=11[press];[3:a]atrim=start=${stageOps[0].source.in}:end=${stageOps[0].source.out},asetpts=PTS-STARTPTS,loudnorm=I=-17:TP=-1.5:LRA=11,adelay=${stageDelays[0]}|${stageDelays[0]}[college];[4:a]atrim=start=${stageOps[1].source.in}:end=${stageOps[1].source.out},asetpts=PTS-STARTPTS,loudnorm=I=-17:TP=-1.5:LRA=11,adelay=${stageDelays[1]}|${stageDelays[1]}[wnba];[5:a]atrim=start=${stageOps[2].source.in}:end=${stageOps[2].source.out},asetpts=PTS-STARTPTS,loudnorm=I=-17:TP=-1.5:LRA=11,adelay=${stageDelays[2]}|${stageDelays[2]}[usa];[6:a]atrim=0:${edl.duration},volume=0.06[music];[vo][press][college][wnba][usa][music]amix=inputs=6:duration=longest:normalize=0,alimiter=limit=0.95[a]`,"-map","0:v","-map","[a]","-t",String(edl.duration),"-c:v","libx264","-preset","veryfast","-crf","18","-r","30","-g","30","-keyint_min","30","-pix_fmt","yuv420p","-c:a","aac","-ar","48000","-b:a","192k","-movflags","+faststart",clean]);
