...
const output={schemaVersion:"1.1",generatedAt:new Date().toISOString(),expectedDuration:481.99,artifacts,durationDelta,streamGap,avSyncFinding:streamGap<=0.1?"Automated duration/stream check passed; video and audio streams cover the same span in both masters. Final human audiovisual review still required before publication.":`FAIL: video and audio stream durations differ by ${streamGap.toFixed(3)}s in at least one master.`,comparisonFinding:durationDelta<=0.05?"PASS: labeled and clean masters have matching durations.":"FAIL: duration mismatch.",cursorArtifacts:files.concat("qc-manifest.json").map(name=>({path:`artifacts/${name}`,sizeBytes:name.endsWith(".json")?null:artifacts.find(item=>item.name===name)?.sizeBytes??null}))};
