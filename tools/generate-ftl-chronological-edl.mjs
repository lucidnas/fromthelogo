...
for (const play of manifest.plays) {
  ...
  // Build to the decision point, hold that exact frame, then resume from that
  // same source timestamp in slow motion through the verified payoff. Never
  // jump forward between the freeze and replay.
  const liveDuration = round(play.freezeFrame - play.sourceIn);
  const slowDuration = round((play.sourceOut - play.freezeFrame) / play.slowMotion.speed);
  const holdDuration = 5;
  const aftermathDuration = round(duration - liveDuration - holdDuration - slowDuration);
  let cursor = 0;
  const operations = [];
  operations.push(op(`${id}-P01`,cursor,cursor+liveDuration,play.sourceIn,play.freezeFrame,1,"live",`Chronological possession setup through Caitlin's verified decision frame.`,playEvidence)); cursor += liveDuration;
  ...
  operations.push(op(`${id}-P03`,cursor,cursor+slowDuration,play.freezeFrame,play.sourceOut,play.slowMotion.speed,"slow-motion-payoff","The chronological possession resumes on the held frame and continues without a source-time jump through the complete verified payoff; this action has not already played live.",playEvidence)); cursor += slowDuration;
...
fs.writeFileSync(path.join(project,"EDL.md"),`# ${manifest.title}\n\nStatus: **truth-approved**  \nRevision: **4**  \nRuntime: **8:01.99**  \nOrder: **strict chronological**  \nCoverage: **4/4 made field goals and 11/11 assists**  \nVO: **Josh Australian at 1.00×; no global speed change**\n\nEach play runs live to its verified decision frame, holds that exact frame for five seconds, then resumes from the identical source timestamp at 0.65× through the complete scoring or assist payoff. There is no timestamp skip between hold and slow motion. Verified broadcast aftermath fills the remainder. The postgame sequence uses three native-audio excerpts from the official USA Basketball upload. The clean and HyperFrames-labeled variants share this exact picture-and-audio contract.\n\n| Beat | Program | Role | Audio | Source |\n|---|---:|---|---|---|\n${rows}\n`);
